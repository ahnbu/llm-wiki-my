...
assert_contains "$PLUGIN_DIR/skills/wiki-manager/references/ingestion.md" "YYYYMMDD_source_file_name_00_프롤로그\\.md|YYYYMMDD_source_file_name" "ingestion protocol documents source_file_name split filenames"
assert_not_contains "$PLUGIN_DIR/commands/ingest.md" "--source-key" "ingest command no longer exposes source-key"
assert_not_contains "$PLUGIN_DIR/skills/wiki-manager/references/ingestion.md" "--source-key" "ingestion reference no longer exposes source-key"
assert_not_contains "$PLUGIN_DIR/skills/wiki-manager/references/ingestion.md" "sourcekey" "ingestion reference no longer uses sourcekey placeholder"
